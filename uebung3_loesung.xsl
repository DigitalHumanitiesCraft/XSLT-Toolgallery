...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:template match="/">
		<html lang="en">
			...
			<body class="container">
				...
				<h2>
					<xsl:value-of select="//t:titleStmt/t:title[@xml:lang = 'de-x-enh']"/>
				</h2>
				<div class="card">
					<div class="card-body">
						<h5 class="card-title">Datierung:</h5>
						<p class="card-text">
							<xsl:value-of select="//t:origDate/t:date[@xml:lang = 'de']"/>
						</p>
						<h5 class="card-title">Herkunft:</h5>
						<p class="card-text">
							<xsl:value-of select="//t:origPlace/t:placeName[@xml:lang = 'de']"/>
						</p>
					</div>
				</div>
				<div class="card">
	...
	<xsl:template match="t:instruction">
		<xsl:result-document href="instructions/{translate(substring(.,1,10),' ','')}.xml"
			method="xml">
			<instruction>
				<xsl:value-of select="."/>
			</instruction>
